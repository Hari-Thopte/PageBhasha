...
async function explain(){if(busy)return;stopReading();if(sampleMode){buildSample();return;}const payload={...prefs(),text:mode==='text'?$('#source-text').value.trim():'',image:mode==='upload'?image:null};if(!payload.image&&payload.text.length<30){error('Add a textbook page or paste at least 30 characters to get started.');return;}error('');setBusy(true);$('#empty-state').hidden=true;$('#lesson-content').hidden=!lesson;$('#loading-state').hidden=false;startThinking(payload);try{const data=await api('https://fmpe1czgw7.execute-api.ap-south-1.amazonaws.com/prod/learn',payload);if(data.status!=='ready'){error(data.summary);return;}lesson={...data,id:crypto.randomUUID(),created:new Date().toISOString()};activeSource=payload;activeView=mode==='text'?{text:payload.text,kind:'text'}:structuredClone(sourceView);chat=[];renderLesson();addHistory();toast('Your lesson is ready.');}catch(e){error(e.message);}finally{stopThinking();setBusy(false);$('#loading-state').hidden=true;if(lesson)renderLesson();else $('#empty-state').hidden=false;}}
...
async function sendQuestion(question,index){
 ...
 try{const result=await api('https://fmpe1czgw7.execute-api.ap-south-1.amazonaws.com/prod/ask',{...activeSource,question,history:previous.slice(-8)});chat.push({role:'assistant',text:result.answer,sources:result.sources||[]});$('#question').value='';}
...
async function playServerTTS(fullText, language, genderPref){
  ...
  try {
    const res = await fetch('https://fmpe1czgw7.execute-api.ap-south-1.amazonaws.com/prod/tts', {
      method: 'POST',
...
function checkStatus(){fetch('https://fmpe1czgw7.execute-api.ap-south-1.amazonaws.com/prod/status').then(r=>r.json()).then(updateConnectionUI).catch(()=>{$('#connection-status').textContent='Server unavailable';});}
checkStatus();
saveHistory();
let pdf=null;
